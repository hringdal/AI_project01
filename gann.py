...
class Gannmodule():
    ...
    def build(self):
        ...
        self.weights = tf.Variable(np.random.uniform(-.2, .2, size=(self.insize,n)),
...
def countex(epochs=100,nbits=15,ncases=500,lrate=0.001,mbs=64,vfrac=0.1,tfrac=0.1,vint=200,sm=True,bestk=1, mapping=True, dendrogram=True, weight_bias=True):
...
def wine(epochs=10000, lrate=0.0001, showint=0, mbs=1024, vfrac=0.1, tfrac=0.1, vint=200, sm=True, bestk=1):
    case_generator = (lambda: load_wine_dataset())
    cman = Caseman(case_generator, vfrac=vfrac, tfrac=tfrac, standardizing=True)
    ann = Gann(dims=[11, 128, 128, 6], cman=cman, lrate=lrate, mbs=mbs, vint=vint, softmax=sm)
    ann.run(epochs,bestk=bestk)
